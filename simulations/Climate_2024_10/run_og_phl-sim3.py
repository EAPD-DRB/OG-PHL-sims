...
def main():
    ...
    reform_dir = os.path.join(save_dir, "reform_3")
    ...
    p2 = copy.deepcopy(p)
    p2.baseline = False
    p2.output_base = os.path.join(reform_dir)
    ...
    # simulation_3:
	# Earning Ability goes down permanently by 1%
	# Labor disutility goes up down permanently by 1% 
	# TFP goes down by 1%

    p2.e = p.e * 0.99
    p2.chi_n = p.chi_n * 1.01
    p2.Z=p.Z*0.99
    ...
    op.plot_all(
        base_dir, reform_dir, os.path.join(save_dir, "OG-PHL_example_plots-reform")
    )

    print("Percentage changes in aggregates:", ans)
    # save percentage change output to csv file
    ans.to_csv(os.path.join(reform_dir, "OG-PHL_example_output.csv"))
